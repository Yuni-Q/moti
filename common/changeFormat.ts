export const changeFormat = (startAt: any, studyingTime: any): string => {
	...
	let date = `${matches[0].replace(/-/g, '. ')} ${weekday[day]} ${hour}:${minute}`;
	...
	if (studyingTime) {
		...
		date += ` ~ ${endHour < 10 ? `${endHour}` : endHour}:${endMinute < 10 ? `0${endMinute}` : endMinute}`;
	}
